...
# 1. Compile
echo "--- step 1/7: mix compile --warnings-as-errors"
mix compile --warnings-as-errors
echo "    PASSED"
...
echo "--- step 1b/7: schema.dict.json drift check"
...
# 2. C1 catalog_parity
echo "--- step 2/7: mix samen.verify.catalog_parity"
mix samen.verify.catalog_parity
echo "    PASSED"

# 3. C2 prefixes
echo "--- step 3/7: mix samen.verify.prefixes"
mix samen.verify.prefixes
echo "    PASSED"

# 4. C3 pii_reads
echo "--- step 4/7: mix samen.verify.pii_reads"
...
echo "--- step 5/7: mix samen.verify.pii_classify"
mix samen.verify.pii_classify --baseline "$COMMITTED_DICT"
echo "    PASSED"

# 6. C5 no_plaintext_pii
echo "--- step 6/7: mix samen.verify.no_plaintext_pii"
