...
- **Status:** **PROPOSED** (design only — the operator ratifies §9 before any batch starts).
- **Date:** 2026-08-14
...
- **Deciders:** the operator, for the seven decisions in **§9** (autonomous-write policy, grant
  ...
  deferral, verifier placement, and the v1 slice). Everything else is recorded design a BATON
  builder executes without re-deriving. Recommendations are marked PROPOSED throughout.
...
| the approval `kind` naming for agent-proposed actions; the deadline default | A4 |
