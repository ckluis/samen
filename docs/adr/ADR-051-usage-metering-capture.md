...
- **Status:** **ACCEPTED (2026-09-25).** Drafted 2026-09-24 as issue #55; the operator took the
  recommended answer on all four decisions (§5: D1 (a), D2 (a), D3 caller `source_ref`, D4 yes).
- **Date:** 2026-09-24 (drafted) · 2026-09-25 (accepted)
- **Build status:** **P1 BUILT** in the change that lands this file: the `UsageEvent` ledger
...
- **Deciders:** the operator, on §5 D1–D4 (decided 2026-09-25).
...
2. **An insert-only event ledger** (`<abbrev>_usage_event`, D1): `metric`, `quantity`,
   ...
   `(org_id, idempotency_key)`. The resource defines **no update and no destroy action at all**.
   A replay of the same event is a no-op that reports `{:ok, :duplicate}`, never a second row and
   never an error the caller must special-case.
   *As built:* the Meter upserts on the identity with **no fields to change**. That returns the
   existing row, and `record/3` tells a replay from a new event by the row id it assigned. A bare
   `ON CONFLICT DO NOTHING` returns no row, and catching a unique violation instead would abort
   the caller's enclosing transaction (Postgres 25P02). R1 pins the in-transaction case.
3. **The idempotency key is the caller's event identity** (D3). `record/3` requires
   ...
   stored key is the first 128 bits of `sha256(metric <> 0x00 <> source_ref)` stamped as an
   RFC 9562 **UUIDv8** (`Meter.idempotency_key/2`), so it is bounded, carries no free text, and
   is the same on every retry by construction. *As built, why a UUID:* the default-deny CDC
   classifier (`Samen.Cdc.Projection`) treats any string column as `:plaintext_pii` unless it
   carries a two-reviewer `non_pii!` clearance. A UUID column is a structural-safe bounded id, so
   the whole ledger mirrors token-blind with no clearance. `source_ref` is never stored. A missing `source_ref` is
