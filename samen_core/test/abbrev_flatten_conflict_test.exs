defmodule Samen.AbbrevFlattenConflictTest do
  ...
  describe "committed registry (zero conflicts)" do
    ...
    test "load/0 does not raise and returns the full lossless union (263 global + 183 host)" do
      flat = Reg.load()
      ...
      # +7 T163 (ADR-051): the usage-capture ledger on every Billing mount = 450.
      assert map_size(flat) == 450
