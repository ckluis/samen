...
- Sabotage harness (opt-in): `SAMEN_SABOTAGE=1 ./ci.sh` or `./scripts/sabotage.sh` —
  ...
  Default (no args) = the full harness (count: `ls scripts/sabotages/*.patch | wc -l` — **356**
  as of 2026-09-21). At that count the full serial run exceeds the 600s single tool-call ceiling,
