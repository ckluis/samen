defmodule Samen.Web.Marketing.LeadsLive do
  ...
  def load(socket, nil) do
    assign(socket, no_org: true, org_id: nil, leads: [])
  end
  ...
  def load(socket, org_id) do
    ...
    assign(socket, no_org: false, org_id: org_id, leads: leads)
  end
  ...
  def render(assigns) do
    ...
    <div id="mkt-leads">
      <.app_shell>
        <:sidebar>
          <.marketing_sidebar mount={@samen_mount} org_id={@org_id} active={:marketing_leads} />
        </:sidebar>

        <.topbar title="Leads" crumbs={crumbs(@samen_mount, "Leads")}>
          <:actions>
