defmodule Samen.Web.Auth.ConfirmLive do
  ...
  On mount, consumes the raw `:token` path param through
  `Samen.Identity.Confirm.consume/2` — a single atomic, single-use, expiring
  token consume (`Samen.Auth.TokenConsume`) that sets `Credential.verified_at`
  on success. `{:error, :invalid_token}` renders the SAME generic "this link
  is invalid or has expired" copy whether the token was never real, already
  consumed, or expired — no distinguishing oracle.
  """
  ...
  def mount(%{"token" => token}, session, socket) do
    ...
    {:ok, assign(socket, verified?: match?({:ok, _}, result), error: consume_error(result))}
  end

  defp consume_error({:error, :invalid_token}), do: "This link is invalid or has expired."
  defp consume_error(_), do: nil
