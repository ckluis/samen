defmodule Samen.Web.Billing.InvoicesLive do
  ...
  def load(socket, nil) do
    assign(socket, no_org: true, org_id: nil, invoices: [], outstanding_cents: 0)
  end
  ...
  def load(socket, org_id) do
    ...
    assign(socket, no_org: false, org_id: org_id, invoices: invs, outstanding_cents: outstanding)
  end
  ...
  def render(assigns) do
    ...
    <div id="billing-invoices">
      <.app_shell>
        <:sidebar>
          <.billing_sidebar mount={@samen_mount} org_id={@org_id} active={:billing_invoices} />
        </:sidebar>

        <.topbar title="Invoices" crumbs={crumbs(@samen_mount, "Invoices")}>
          <:actions>
