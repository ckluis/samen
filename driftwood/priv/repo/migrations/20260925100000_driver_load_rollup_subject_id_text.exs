defmodule Driftwood.Repo.Migrations.DriverLoadRollupSubjectIdText do
  @moduledoc """
  Issue #47: `drl_subject_id` widens from `UUID` to `TEXT`.
  ...
  def up do
    execute "ALTER TABLE drl_driver_load_count ALTER COLUMN drl_subject_id TYPE TEXT USING drl_subject_id::text"

    execute """
    UPDATE fld_field SET fld_type = 'String'
    WHERE fld_table_name = 'drl_driver_load_count' AND fld_column_name = 'drl_subject_id'
    """
  end
  ...
  def down do
    execute "ALTER TABLE drl_driver_load_count ALTER COLUMN drl_subject_id TYPE UUID USING drl_subject_id::uuid"
