defmodule Driftwood.Repo.Migrations.AudChain do
  ...
  Mirrors `Demo.Repo.Migrations.AudChain` / `SamenCore.TestRepo.Migrations.AudChain`
  — same DDL, same append-only enforcement (role REVOKE + trigger), same catalog
  rows in the same migration transaction.

  ## Why Driftwood needs this (T5.4 finding)
  ...
  requires the chain to actually exist, so this migration deploys it — the same table
  the demo has. `ach_detail` is the deliberately-preserved plaintext token channel
  (ADR-002 §2.5); `ach_subject_ciphertext` is the per-subject key-destroyable column
  that makes the chain crypto-shreddable.
