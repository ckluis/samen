...
| Kernel **Invoice** attrs: `stripe_invoice_id, status, amount_due_cents, amount_paid_cents, currency, period_start/end, due_date, paid_at, line_items ({:array,:map}), custom`; `belongs_to :customer, :subscription`. **Money is integer cents.** | `billing/blueprint.ex` `define_invoice/7` |
